...
const menuGroups = [
  ...
  },
  {
    ...
    items: [{ label: 'Orçamentos', path: 'orcamentos' }, { label: 'Pedidos', path: 'pedidos' }]
  },
  {
    label: 'Operacional',
    icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 1 1-3.586-3.586l5.653-4.655m5.908-6.258a6.375 6.375 0 0 1-8.955 8.955" /></svg>,
    items: [
  ...
  },
  {
    ...
    items: [
      { label: 'Educação Financeira', path: 'educacao' },
      { label: 'Simulador de Preços', path: 'precificacao' },
    ]
  },
  {
    label: 'Inteligência',
...
export function Sidebar({ onNavigate }) {
  ...
  const getInitialOpen = () => {
    ...
    menuGroups.forEach(group => {
      if (group.items.some(item => location.pathname.includes(`/${item.path}`))) open[group.label] = true
    })
    return open
  }
          ...
          return (
            <div key={group.label} className="mb-1">
              <button onClick={() => toggleGroup(group.label)}
                className={`w-full flex items-center justify-between px-4 py-2.5 text-sm font-medium transition-colors ${isGroupActive ? 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                <div className="flex items-center gap-3">
                  <span className={isGroupActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}>{group.icon}</span>
